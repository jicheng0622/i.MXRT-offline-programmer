...
void USBD_Init(void)
{
    ...
    SIM->CLKDIV2 = (uint32_t)0x09UL; /* Update USB clock prescalers, 120M*2/5=48MHz */
    /* Enable USB-OTG IP clocking */
    SIM->SCGC4 |= (SIM_SCGC4_USBOTG_MASK);
    /* Configure enable USB regulator for device */
    SIM->SOPT1CFG |= SIM_SOPT1CFG_URWE_MASK;//Allow to write USBREGEN bit in SOPT1
    SIM->SOPT1 |= SIM_SOPT1_USBREGEN_MASK;//Enable USB Regulator
    ...
    USB0->INTEN = USB_INTEN_USBRSTEN_MASK |
            ...
            ((USBD_RTOS_DevTask   != 0) ? USB_INTEN_SOFTOKEN_MASK : 0) |
            ((USBD_RTOS_DevTask   != 0) ? USB_INTEN_ERROREN_MASK  : 0) ;
#else
...
void USBD_Handler(void)
{
    ...
    uint8_t suspended = 0;
#ifdef USBSTACK_RTOS_FREE_RTOS
    BaseType_t xHigherPriorityTaskWoken = pdFALSE;//Use for context switch flag on ISR exit
#endif
    ...
    if (istr & USB_ISTAT_USBRST_MASK) {
        ...
        if (USBD_RTOS_DevTask) {
            xTaskNotifyFromISR(USBD_RTOS_DevTask, USBD_EVT_RESET, eSetBits, &xHigherPriorityTaskWoken);
        }
    ...
    if (istr & USB_ISTAT_SLEEP_MASK) {
        ...
        if (USBD_RTOS_DevTask) {
            xTaskNotifyFromISR(USBD_RTOS_DevTask, USBD_EVT_SUSPEND, eSetBits, &xHigherPriorityTaskWoken);
        }
    ...
    if (istr & USB_ISTAT_RESUME_MASK) {
        ...
        if (USBD_RTOS_DevTask) {
            xTaskNotifyFromISR( USBD_RTOS_DevTask, USBD_EVT_RESUME, eSetBits, &xHigherPriorityTaskWoken);
        }
    ...
    if (istr & USB_ISTAT_SOFTOK_MASK) {
        ...
        if (USBD_RTOS_DevTask) {
            xTaskNotifyFromISR(USBD_RTOS_DevTask, USBD_EVT_SOF, eSetBits, &xHigherPriorityTaskWoken);
        }
    ...
    if (istr == USB_ISTAT_ERROR_MASK) {
        ...
        if (USBD_RTOS_DevTask) {
            xTaskNotifyFromISR(USBD_RTOS_DevTask, USBD_EVT_ERROR, eSetBits, &xHigherPriorityTaskWoken);
        }
    ...
    if (istr & USB_ISTAT_TOKDNE_MASK) {
        while (!stat_is_empty()) {
            ...
            if ((num == 0) && (TOK_PID((IDX(num, dir, ev_odd))) == SETUP_TOKEN)) {
                ...
                if (USBD_RTOS_EPTask[num]) {
                    xTaskNotifyFromISR(USBD_RTOS_EPTask[num], USBD_EVT_SETUP, eSetBits, &xHigherPriorityTaskWoken);
                }
                ...
                if (TOK_PID((IDX(num, dir, ev_odd))) == OUT_TOKEN) {
                    ...
                    if (USBD_RTOS_EPTask[num]) {
                        xTaskNotifyFromISR(USBD_RTOS_EPTask[num], USBD_EVT_OUT, eSetBits, &xHigherPriorityTaskWoken);
                    }
                ...
                if (TOK_PID((IDX(num, dir, ev_odd))) == IN_TOKEN) {
                    ...
                    if (USBD_RTOS_EPTask[num]) {
                        xTaskNotifyFromISR(USBD_RTOS_EPTask[num], USBD_EVT_IN, eSetBits, &xHigherPriorityTaskWoken);
                    }
