...
void USBD_MSC_BulkOut(void)
{
    switch (BulkStage) {
        ...
        case MSC_BS_CSW:
            ...
            assert(USBD_EndPointHalt & (1 << usbd_msc_ep_bulkout));
            break;
...
#ifdef USBSTACK_RTOS_FREE_RTOS                       /* RTOS tasks for handling events */
 ...
 *    Return Value:    None
 */
void USBD_RTOS_MSC_EP_BULKIN_Event(void)
{
    for (;;) {
        if (ulTaskNotifyTake(pdTRUE, portMAX_DELAY) & USBD_EVT_IN) {
            USBD_MSC_EP_BULKIN_Event(0);
        }
    }
}
 ...
 *    Return Value:    None
 */
void USBD_RTOS_MSC_EP_BULKOUT_Event(void)
{
    for (;;) {
        if (ulTaskNotifyTake(pdTRUE, portMAX_DELAY) & USBD_EVT_OUT) {
            USBD_MSC_EP_BULKOUT_Event(0);
        }
    }
}
...
void USBD_RTOS_MSC_EP_BULK_Event(void)
{
    for (;;) {
        USBD_MSC_EP_BULK_Event(ulTaskNotifyTake(pdTRUE, portMAX_DELAY));
    }
}
#endif
